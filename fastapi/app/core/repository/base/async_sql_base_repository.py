...
class AsyncSQLBaseRepository(CRUDRepositoryInterface):
    ...
    def __init__(
        ...
        self.db_session = db_async_session

    async def index(self, paginate_data=False, page_params: Params = None):
    ...
    async def update_by_id(self, obj_id, obj_in):
        db_obj = await self.find_by_id(obj_id)
        async with self.db_session() as session:
    ...
    async def find(self, filter_param: dict):
        async with self.db_session() as session:
    ...
    async def update(self, filter_params, obj_in):
        db_obj = await self.find(filter_params)
        async with self.db_session() as session:
            for field, value in obj_in.items():
                if hasattr(db_obj, field):
                    setattr(db_obj, field, value)
    ...
    async def delete(self, filter_params: dict):
        db_obj = await self.find(filter_params)
        async with self.db_session() as session:
            await session.delete(db_obj)
            await session.commit()
    ...
    async def _result(self, conn, query, paginated=False, page_params=None, many=True):
        if paginated and page_params:
            return await paginate(conn=conn, query=query, params=page_params)
        results = (await conn.execute(query)).scalars()
        if not many:
